...
bitset<48> subKey(bitset<56> key, int round) {
    ...
    if (round == 15) { // Initialize C and D only on the last round
        C = bitset<28>(key.to_ullong() >> 28); // Left 28 bits
        D = bitset<28>(key.to_ullong() & 0xFFFFFFF); // Right 28 bits
    }

    // Rotate C and D to the right by the number of shifts specified for this round
    for (int i = 0; i < ROTATE_SHIFTS[15 - round]; i++) {
        C = (C >> 1) | (C << 27); // Rotate right
        D = (D >> 1) | (D << 27); // Rotate right
    }
...
bitset<64> desDecryption(bitset<64> cipherText, bitset<64> key){
    string combinedHalves;
    //TODO ^ may need? leave for now
    vector<bitset<48>> subKeyV;

    // Perform initial permutation inverse
    bitset<64> permutedBlock = finalPermutation(cipherText);
    ...
    //Generate full subkey Vector
    for(int i = 0; i < 16; i++){
        bitset<48> subKeyValue = subKey(key56, i);
        subKeyV.push_back(subKeyValue);
    }

    for (int round = 0; round < 16; round++){

        // Apply the F function
        bitset<32> fResult = fFunction(rightHalf, subKeyV.at(16 - round - 1));
    ...
    return initialPermutation(finalBlock);
}

int main() {
    // Hex values for the DES parameters
    string hexCiphertext, hexKey;
    cout << "Enter ciphertext: ";
    cin >> hexCiphertext;
    cout << "Enter key: ";
    cin >> hexKey;
    
    bitset<64> plainText = hexToBitset64(hexCiphertext);
    bitset<64> key = hexToBitset64(hexKey);
    
    bitset<64> decrypted = desDecryption(plainText, key);
    
    cout << "Decrypted Data: " << bitset64ToHex(decrypted) << endl;
    return 0;
}